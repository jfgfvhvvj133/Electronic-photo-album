...
int display_bmp2(int x0,int y0,char* bmpfile)
{
    ...
    unsigned char color_buf[total_bytes];       // 存像素数组
    lseek(fd,0x36,SEEK_SET);                    // 将光标移动到像素数组初始位置
    read(fd,color_buf,total_bytes);             // 从文件像素数组中读取颜色值

    // 在屏幕上显示
    int color;
    int i = total_bytes-1;                      // 要从后往前显示将i取像素数组最大值
    unsigned char a,b,g,r;                      // 保存各颜色值的变量
    for(int y = abs(height);y > 0;y--)          // 从像素数组中读取颜色值
    {
        // 每一行的末尾,如果字节数不是4的倍数,需要填充几个字节
        i = i - fill_bytes;
...
int display_bmp3(int x0,int y0,char* bmpfile)
{
    ...
    int i = total_bytes-1;                      // 要从后往前显示将i取像素数组最大值
    unsigned char a,b,g,r;                      // 保存各颜色值的变量
    for(int x = 0;x < width;x++)                // 从像素数组中读取颜色值
    {
        // i = i - fill_bytes;
        ...
        for(int y = 0;y < abs(height);y++)
        {
            ...
            lcd_draw_point(width-(x+x0),y+y0,color);    // 上色    
        }
...
int display_bmp4(int x0,int y0,char* bmpfile)
{
    ...
    for(int x = 0;x < width;x++)                // 从像素数组中读取颜色值
    {
        ...
        if(depth == 32)                         // 找下一列元素
        {
            i = (total_bytes-1) - 4*(width-x);
        }
        else
        {
            i = (total_bytes-1) - 3*(width-x);
        }
